...
object SetExample {
  def main(args: Array[String]): Unit = {
    ...
    // 4. Creating a mutable Set
    val marks: Set[StMarks] = Set()
    
    // 5. Adding a single `StMarks` record to the Set
    marks += StMarks("Faith", "F", 9, 11, 33)
    
    // 6. Adding multiple `StMarks` records to the Set
    marks ++= Set(
    ...
    // 7. Printing the first record (arbitrarily since Set doesn't have an order)
    println("Head of set (arbitrary): " + marks.head)
    
    // 8. Printing all records in the set
