...
object SequenceExample {
  def main(args: Array[String]): Unit = {
    ...
    // 4. Creating a Seq (immutable)
    val marks: Seq[StMarks] = Seq()
    
    // 5. Adding a single `StMarks` record to the Seq
    val marksUpdated = marks :+ StMarks("Faith", "F", 9, 11, 33)
    
    // 6. Adding multiple `StMarks` records to the Seq
    val moreMarks = marksUpdated ++ Seq(
    ...
    // 7. Printing the first record in the seq
    println("Head of seq: " + moreMarks.head)
    
    // 8. Printing all records in the seq
